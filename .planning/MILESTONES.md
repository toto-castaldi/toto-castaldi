# Milestones

## v1.2 Rimozione Helix (Shipped: 2026-07-19)

**Phases completed:** 1 phases, 3 plans, 4 tasks

**Key accomplishments:**

- Removed the Helix project from all shipped artifacts — project data, IT/EN SEO descriptions, and repo docs — leaving a clean two-project (Docora, Lumio) Hugo build with zero Helix matches.
- Automated build/grep gates confirmed zero Helix references and a clean 2-column build, then the user visually approved the running site (two cards, 2-column desktop, single-column mobile, WCAG AA contrast in both themes across IT/EN) to ship v1.2.

---

## v1.0 MVP (Shipped: 2026-02-19)

**Phases completed:** 3 phases, 6 plans | 52 files, 501 LOC | 2 days
**Git range:** 359cded..723ae46

**Key accomplishments:**

- Hugo bilingual scaffold with symmetric /it/ and /en/ URL structure
- GitHub Actions CI/CD pipeline + GitHub Pages custom domain deploy
- Clean responsive design with CSS-only dark mode toggle (zero JS)
- Bilingual project cards for Docora, Lumio, Helix with system font stack
- hreflang + Open Graph + canonical URLs for multilingual SEO
- HTTPS with Let's Encrypt certificate on toto-castaldi.com

---

## v1.1 Restyling Bento Grid (Shipped: 2026-02-19)

**Phases completed:** 3 phases (4-6), 3 plans, 6 tasks | 21 files changed, +2248/-89 | 489 LOC
**Git range:** 9e1fde3..3eaa104

**Key accomplishments:**

- WCAG AA color tokens in both themes (text 4.5:1+, border 3:1+) with Tailwind Slate palette
- Off-white/white card distinction and 1080px container for bento grid
- Responsive CSS Grid: 3 columns desktop, 2 tablet, 1 mobile with design tokens
- Shadow depth tokens and accessible hover effects with prefers-reduced-motion guard
- GitHub profile bento section with inline SVG and bilingual i18n support

---
